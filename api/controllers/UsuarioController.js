...
class UsuarioController{
    ...
    createRecovery(req,res,next){
        ...
        Usuario.findOne({email}).then((usuario)=>{
            if(!usuario) return res.render('recovery',{error: 'Não exite usuário com este email!!!',success:null});
    ...
    showCompleteRecovery(req, res, next){
        if(!req.query.token) return res.render('recovery', {erro: 'Token não identificado', success:null});
        Usuario.findOne({'recovery.token':req.query.token}).then(usuario=>{
            if(!usuario)return res.render('recovery', {error: 'Não exite usuário com esse email', success: null});
